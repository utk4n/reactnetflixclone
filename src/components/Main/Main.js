...
const Main = () => {
  ...
  useEffect(() => {
    axios.get(apiRequests.popularMovies).then((res) => setMovies(res.data.results));
  }, []);
  const movie = movies[Math.floor(Math.random() * movies.length )]
//   console.log(movie);

const dotString = (str, num) => {
    if(str?.length > num){
  ...
  return <div className="w-full h-[550px] text-white ">
    ...
    <p className="w-full md:max-w-[70%] lg:max-w-[50%] xl:max-w-[35%] text-gray-200">{dotString(movie?.overview, 180)}</p>
    </div>
  </div>;
};
